...
--      逻辑: repay_type <> 8 排除追偿；repay_type=7（代偿）时间<=2025-08-31 或身份证尾号为1/4按正常还款报送
...
USING (
    ...
    JOIN prod_dw_01.dim_base_indiv_info_incr_t f
        ON b.indiv_cust_id = f.indiv_cust_id
    -- 还款信息报送策略：
    ...
    WHERE
        -- 排除所有追偿（repay_type = 8）
        a.repay_type <> 8
        -- 代偿(repay_type=7)时间在2025年9月30日之前的记录；身份证尾号为1/4的本月代偿按正常报送
        AND (a.repay_type <> 7 OR a.repay_time <= '2025-08-31'
              OR SUBSTR(f.id_card, -1, 1) IN ('1', '4')
             )
        AND e.is_use = 'Y'
